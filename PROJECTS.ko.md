...
* **문서는 코드의 부산물이 아니라 선행 조건.** 조용히 고르면 나중에 되돌릴 수 없는 선택 — 아키텍처·계약 리터럴·정책 — 은 코드를 쓰기 전에 선택지 표를 갖춘 **결정 브리프**로 올리고, 오너가 결정할 때까지 구현을 멈춥니다. 현재까지 **결정 브리프 80여 건**. 구현 뒤에는 **다른 세션**이 **뮤테이션**으로 반증을 시도합니다 — 고친 것을 되돌려 회귀 가드가 다시 실패하는지 확인하는 방식입니다. 가드는 양방향이어야 합니다: 원래 결함을 재현해도 실패하고, 과잉 교정으로 정상 경로를 깨도 실패해야 합니다.
* **실패가 허용되는 검증.** **50일 넘게 쌓인 독립 검증 기록 240여 건**, 그 아래 **테스트 2,000여 건**. **검증 네 건 중 한 건꼴로 '조건부 합격'**이 나오고 불합격도 나옵니다. **이 비율이 절차가 형식적 통과가 아니라는 증거**이며, 각 지적은 후속 커밋에서 닫힙니다. 대표적으로 잡힌 것 하나: "시행 완료"로 기록된 항목이 **compose 파일 수준에서는 참이고 런타임에서는 거짓**이었습니다 — 이미 만들어진 컨테이너가 옛 포트 매핑을 그대로 들고 있었기 때문이고, 이를 잡은 것은 테스트 스위트가 아니라 `docker ps`였습니다.
* **범위의 정직한 경계.** 현재 단계는 **로컬 1인 운영(dogfood 직전)**입니다. 전체 스택이 `docker compose up` 하나로 관통 동작하며(FastAPI · React/TS · MongoDB · ChromaDB · Elasticsearch · 로컬 Gemma 4 12B endpoint), 다중 사용자 인증·프로젝트 소유권·요청 quota·관리자 표면까지 구현돼 있습니다. 아직 **아닌 것**: 실사용자를 받는 다중 테넌트 프로덕션 서비스.

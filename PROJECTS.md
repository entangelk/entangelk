...
* **Documentation as a Precondition, Not a Byproduct.** Choices that cannot be quietly reversed later — architecture, contract literals, policy — are raised as a **decision brief** with an options table before any code is written, and implementation stops until the owner decides. **80+ decision briefs** exist to date. After implementation, a *different* session attempts to falsify the work by **mutation**: reverting the fix to confirm the regression guard fails again. Guards must fail in both directions — reintroducing the original bug, and over-correcting into a valid case.
* **Verification That Is Allowed to Fail.** **240+ independent verification records over 50+ days**, sitting on a suite of **2,000+ tests**. Roughly **one verification in four comes back *conditional*** and a couple have failed outright — that rate **is the evidence the process is real** rather than ceremonial; each finding is closed in a follow-up commit. One representative catch: an enforcement recorded as "complete" was true at the compose-file level but **false at runtime**, because already-created containers still carried the old port mapping — caught by `docker ps`, not by the test suite.
* **Honest Scope Boundary.** Currently at **local single-operator stage (just before dogfooding)**. The stack runs end-to-end on one `docker compose up` (FastAPI · React/TS · MongoDB · ChromaDB · Elasticsearch · a local Gemma 4 12B endpoint), with multi-user auth, project ownership, per-request quota, and an admin surface implemented. What it is **not** yet: a multi-tenant production service with real users.
